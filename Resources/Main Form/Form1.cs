...
namespace ManageMaterialPBA
{
    public partial class Form1 : Form
    {
        ...
        public bool nhapMkrp = false, inpMore = false;
        ...
        public int sttKW = 0;
        //KTZ-other
        DataTable dtKtzOther, dt1KtzOther;
        public bool arrgPerKO;
        public string partEnterKO;
        public bool nhapCodeKO = false;
        public bool chkScCode_KO = true;
        public int sttKO = 0;
        ...
        public string GetLink()
        {
            ...
            StreamReader sr = new StreamReader(@Application.StartupPath + "\\Link_dataCopy.txt");
        ...
        public void CopyDirectory(string strDirSource, string strDirDestination)
        {
            //Database
            File.Copy(strDirSource + "\\Database.mdb", strDirDestination + "\\Database.mdb");
            //BOM
            CopyFile(strDirSource, strDirDestination, "BOM");            
            //History
            CopyFile(strDirSource, strDirDestination, "History");
            CopyFile(strDirSource + "\\History\\In_Out\\Ktz_to_PD", strDirDestination + "\\History\\In_Out\\Ktz_to_PD");
            CopyFile(strDirSource + "\\History\\In_Out\\PD_return_Ktz", strDirDestination + "\\History\\In_Out\\PD_return_Ktz");
            CopyFile(strDirSource + "\\History\\In_Out\\PD_xacnhan", strDirDestination + "\\History\\In_Out\\PD_xacnhan");
            CopyFile(strDirSource + "\\History\\In_Out\\Ktz_other", strDirDestination + "\\History\\In_Out\\Ktz_other");
            CopyFile(strDirSource + "\\History\\WH\\Order", strDirDestination + "\\History\\WH\\Order");
            CopyFile(strDirSource + "\\History\\WH\\Return", strDirDestination + "\\History\\WH\\Return");
            //Log
            CopyFile(strDirSource, strDirDestination, "Log\\Duplicate");
            CopyFile(strDirSource, strDirDestination, "Log\\Duplicate\\Old"); 
            //tem
            CopyFile(strDirSource, strDirDestination, "tem");            
        }

        public void CopyFile(string pthSr, string pthDes, string folNam)
        {
            if (!Directory.Exists(pthDes + "\\" + folNam))
            {
                Directory.CreateDirectory(pthDes + "\\" + folNam);
            } 
            string[] str = Directory.GetFiles(pthSr + "\\" + folNam);
            foreach (string s in str)
            {
                string filName = Path.GetFileName(s);
                File.Copy(pthSr + "\\" + folNam + "\\" + filName, pthDes + "\\" + folNam + "\\" + filName);
            }
        }

        public void CopyFile(string pthSr, string pthDes)
        {
            string[] folNam = Directory.GetDirectories(pthSr);
            for(int i = 0; i < folNam.Length; i++)
            {
                string[] strFolNam = folNam[i].Split('\\');
                string subFolNam = strFolNam[strFolNam.Length - 1];

                if (!Directory.Exists(pthDes + "\\" + subFolNam))
                {
                    Directory.CreateDirectory(pthDes + "\\" + subFolNam);
                }
                string[] str = Directory.GetFiles(pthSr + "\\" + subFolNam);
                foreach (string s in str)
                {
                    string filName = Path.GetFileName(s);
                    File.Copy(pthSr + "\\" + subFolNam + "\\" + filName, pthDes + "\\" + subFolNam + "\\" + filName);
                }
            }            
        }
        ...
        {            
            ...
            ((Control)this.KTZother).Enabled = false;
            tabControl1.TabPages.Remove(KTZother);
            //Khoa them tai khoan
            ...
            //Folder history
            if (!System.IO.Directory.Exists(@Application.StartupPath + "\\History"))
            {
                System.IO.Directory.CreateDirectory(@Application.StartupPath + "\\History");
            }
            if (!System.IO.Directory.Exists(@Application.StartupPath + "\\History\\In_Out"))
            {
                System.IO.Directory.CreateDirectory(@Application.StartupPath + "\\History\\In_Out");
            }
            if (!System.IO.Directory.Exists(@Application.StartupPath + "\\History\\WH"))
            {
                System.IO.Directory.CreateDirectory(@Application.StartupPath + "\\History\\WH");
            }
            //Luu data daily
            string strPath = GetLink();
            if (!Directory.Exists(strPath + "\\" + datTim))
            {
                Directory.CreateDirectory(strPath + "\\" + datTim);
                CopyDirectory(@Application.StartupPath, strPath + "\\" + datTim);
            }
        }
            ...
            if (btn_login.Text == "Đăng nhập")
            {
                ...
                else if (dtb.login_admin(txt_user.Text, txt_pass.Text, txt_IDemploy.Text, kind) == true) // admin đăng nhập
                {
                    ...
                    if (Convert.ToDateTime(dtb.get_DateChangepass(txt_user.Text, txt_pass.Text)) < DateTime.Now.AddDays(-30))
                    {
                        ...
                        if (Isopen == false)
                        {
                            Change_pw changPw = new Change_pw(this, txt_user.Text);
                            changPw.Show();
                        }
                    }
                }
        ...
        private void bOMListToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            if (Isopen == false)
            {
                BOM bom = new BOM(txt_user.Text, txt_pass.Text);
                bom.Show();
            }
        }
        ...
        private void truyXuấtLịchSửToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (Isopen == false)
            {
                TraceHistory hs = new TraceHistory();
                hs.Show();
            }
        }
        ...
        private void inoutMaterialToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            //{
                ...
                if (Isopen == false)
                {
                    Barcode ab = new Barcode(txt_user.Text, txt_pass.Text);
                    ab.Show();
                }              
            //}
            //else
        ...
        private void inCodeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if ((part == "admin" || part == "manager" || part == "KTZ") && ((Control)this.KTZlayNVLWH).Enabled == false)
            {
                ...
                txtQtRow.Enabled = false;
                txtQtRow.Visible = false;
                txtQtRow.Text = "";
                lblQtRow.Visible = false;
                lblQtRow.Enabled = false;
                ...
                //hinh anh
                picBx1.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                picBx1.SizeMode = PictureBoxSizeMode.StretchImage;
                picBx2.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                picBx2.SizeMode = PictureBoxSizeMode.StretchImage;
                //History folder
                if (!System.IO.Directory.Exists(@Application.StartupPath + "\\History\\WH\\Order\\" + dMon))
                {
                    System.IO.Directory.CreateDirectory(@Application.StartupPath + "\\History\\WH\\Order\\" + dMon);
                }
                //clear dgv
        ...
        private void wHKTZToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if ((part == "admin" || part == "manager" || part == "KTZ") && ((Control)this.KTZgiaoNVLLine).Enabled == false)
            {
                ...
                //Hinh anh
                picBx1KP.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                picBx1KP.SizeMode = PictureBoxSizeMode.StretchImage;
                picBx2KP.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                ...
                //History folder
                if (!System.IO.Directory.Exists(@Application.StartupPath + "\\History\\In_Out\\Ktz_to_PD\\" + dMon))
                {
                    System.IO.Directory.CreateDirectory(@Application.StartupPath + "\\History\\In_Out\\Ktz_to_PD\\" + dMon);
                }
                //clear dgv
        ...
        private void lineKTZToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if ((part == "admin" || part == "manager" || part == "PD") && ((Control)this.Linexacnhan).Enabled == false)
            {
                ...
                //hinh anh
                picPDxn1.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                picPDxn1.SizeMode = PictureBoxSizeMode.StretchImage;
                picPDxn2.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                ...
                //History folder
                if (!System.IO.Directory.Exists(@Application.StartupPath + "\\History\\In_Out\\PD_xacnhan\\" + dMon))
                {
                    System.IO.Directory.CreateDirectory(@Application.StartupPath + "\\History\\In_Out\\PD_xacnhan\\" + dMon);
                }
                //clear dgv
        ...
        private void kTZToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if ((part == "admin" || part == "manager" || part == "PD") && ((Control)this.LinetraNVLKTZ).Enabled == false)
            {
                ...
                //hinh anh
                picBx1PK.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                picBx1PK.SizeMode = PictureBoxSizeMode.StretchImage;
                picBx2PK.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                ...
                //History folder
                if (!System.IO.Directory.Exists(@Application.StartupPath + "\\History\\In_Out\\PD_return_Ktz\\" + dMon))
                {
                    System.IO.Directory.CreateDirectory(@Application.StartupPath + "\\History\\In_Out\\PD_return_Ktz\\" + dMon);
                }
                //clear dgv
        ...
        private void kTZTrảNVLWHToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if ((part == "admin" || part == "manager" || part == "KTZ") && ((Control)this.KTZtraNVLWH).Enabled == false)
            {
                ...
                //hinh anh
                picBoxRe1.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                picBoxRe1.SizeMode = PictureBoxSizeMode.StretchImage;
                picBoxRe2.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                picBoxRe2.SizeMode = PictureBoxSizeMode.StretchImage;
                //History folder
                if (!System.IO.Directory.Exists(@Application.StartupPath + "\\History\\WH\\Return\\" + dMon))
                {
                    System.IO.Directory.CreateDirectory(@Application.StartupPath + "\\History\\WH\\Return\\" + dMon);
                }
                //clear dgv
        ...
        private void kTZCấpNVLOrderToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if ((part == "admin" || part == "manager" || part == "KTZ") && ((Control)this.KTZother).Enabled == false)
            {
                sttKO = 0;
                ((Control)this.KTZother).Enabled = true;
                tabControl1.TabPages.Add(KTZother);
                tabControl1.SelectedTab = tabControl1.TabPages["KTZother"];
                //Load data
                dtb1.get_cbbModel("Model", "Model_name", cbx_modelKO);
                cbx_bpKO.Items.Add("CPE");
                cbx_bpKO.Items.Add("ME");
                cbx_bpKO.Items.Add("QA");
                //Hide input manual
                txt_manualKO.Hide();
                txt_manualKO.Enabled = false;
                btn_enterKO.Hide();
                btn_enterKO.Enabled = false;
                //con trỏ chuột đếm tgian out
                posX = Cursor.Position.X;
                posY = Cursor.Position.Y;
                //Get thông tin khi đổi password
                ...
                if (dtNewPass.Rows.Count > 0)
                {
                    partEnterKO = dtb1.get_PerLogin(txt_user.Text, dtNewPass.Rows[0].ItemArray[0].ToString(), "part");
                    arrgPerKO = dtb1.get_RightLogin(txt_user.Text, dtNewPass.Rows[0].ItemArray[0].ToString());
                }
                else
                {
                    partEnterKO = dtb1.get_PerLogin(txt_user.Text, txt_pass.Text, "part");
                    arrgPerKO = dtb1.get_RightLogin(txt_user.Text, txt_pass.Text);
                }
                //hinh anh
                picBox_1KO.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                picBox_1KO.SizeMode = PictureBoxSizeMode.StretchImage;
                picBox_2KO.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                picBox_2KO.SizeMode = PictureBoxSizeMode.StretchImage;
                //History folder
                if (!System.IO.Directory.Exists(@Application.StartupPath + "\\History\\In_Out\\Ktz_other\\" + dMon))
                {
                    System.IO.Directory.CreateDirectory(@Application.StartupPath + "\\History\\In_Out\\Ktz_other\\" + dMon);
                }
                //clear dgv
                dgv_KO.Columns.Clear();
            }
            else
            {
                MessageBox.Show("Bạn không có quyền truy cập hạng mục này\nHoặc trang dữ liệu đã được mở rồi!", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btn_clsKO_Click(object sender, EventArgs e)
        {
            ((Control)this.KTZother).Enabled = false;
            tabControl1.TabPages.Remove(KTZother);
            cbx_modelKO.Text = "";
            ...
            txt_manualKO.Text = "";
            txt_autoKO.Text = "";
            chkb_nhtayKO.Checked = false;
            rbtn_reloadKO.Checked = false;
            chkScCode_KO = true;
            dgv_KO.Columns.Clear();
        }
        #endregion        
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            {             
                ...
                if (((Control)this.KTZlayNVLWH).Enabled == true)
                {
                    ...
                    {
                        excel.ExportTxt(dgv_WH_Ktz, @Application.StartupPath + "\\tem\\" + cbx_Model_OWK.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadWH-KTZ.txt");
                    }
                    //close tab
                    ((Control)this.KTZlayNVLWH).Enabled = false;
                    tabControl1.TabPages.Remove(KTZlayNVLWH);
                ...
                if (((Control)this.KTZgiaoNVLLine).Enabled == true)
                {
                    ...
                    {
                        excel.ExportTxt(dgv_Ktz_Pd, @Application.StartupPath + "\\tem\\" + cbx_ModelKP.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadKTZ-PD.txt");
                    }
                    //close tab
                    ((Control)this.KTZgiaoNVLLine).Enabled = false;
                    tabControl1.TabPages.Remove(KTZgiaoNVLLine);
                ...
                if (((Control)this.Linexacnhan).Enabled == true)
                {
                    ...
                    {
                        excel.ExportTxt(dgv_PDxn, @Application.StartupPath + "\\tem\\" + cbx_modelPDxn.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadPDxacnhan.txt");
                    }
                    //close tab
                    ((Control)this.Linexacnhan).Enabled = false;
                    tabControl1.TabPages.Remove(Linexacnhan);
                ...
                if (((Control)this.LinetraNVLKTZ).Enabled == true)
                {
                    ...
                    {
                        excel.ExportTxt(dgv_Pd_Ktz, @Application.StartupPath + "\\tem\\" + cbx_ModelPK.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadPD-KTZ.txt");
                    }
                    //close tab
                    ((Control)this.LinetraNVLKTZ).Enabled = false;
                    tabControl1.TabPages.Remove(LinetraNVLKTZ);
                ...
                if (((Control)this.KTZtraNVLWH).Enabled == true)
                {
                    ...
                    {
                        excel.ExportTxt(dgv_returnWH, @Application.StartupPath + "\\tem\\" + cbx_reMol.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadReturnWH.txt");
                    }
                    //close tab
                    ((Control)this.KTZtraNVLWH).Enabled = false;
                    tabControl1.TabPages.Remove(KTZtraNVLWH);
                ...
                //KTZ-Other
                if (((Control)this.KTZother).Enabled == true)
                {
                    if (dgv_KO.Columns.Count > 0)
                    {
                        excel.ExportTxt(dgv_KO, @Application.StartupPath + "\\tem\\" + cbx_modelKO.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadKtzOther.txt");
                    }
                    //close tab
                    ((Control)this.KTZother).Enabled = false;
                    tabControl1.TabPages.Remove(KTZother);
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (cfrm == true && inpMore == true)
            {
                ...
                txtQtRow.Enabled = true;
                txtQtRow.Visible = true;
                txtQtRow.Text = "";
                lblQtRow.Visible = true;
                lblQtRow.Enabled = true;
            }
            ...
            if(cfrm == true && nhapCodeKO == true)
            {
                ...
                txt_manualKO.Visible = true;
                txt_manualKO.Enabled = true;
                txt_manualKO.Focus();
                btn_enterKO.Visible = true;
                btn_enterKO.Enabled = true;
                lbl_inpKO.Text = "Nhập Code";
            }
        }
        ...
        private void btn_inputManualWk_Click(object sender, EventArgs e)
        {
            ...
            {                
                ...
                {
                    txt_manualInputWk.Text = crtUpperChar(txt_manualInputWk.Text);
                    if (arrgPerWK == false)
                    {
        ...
        public void chek_input(TextBox txt)
        {
            ...
            {   
                //Check trung code dang input ko
                if(dtb1.CheckSameCode(infFromBom[1], infFromBom[2], infFromBom[3], dgv_WH_Ktz) == true)
                {                    
                    ...
                    {                        
                        ...
                        for (int i = 0; i < rel_con; i++)
                        {
                            sttWK++;
                            dtb1.insert_transOrderWH2v2(sttWK.ToString(), cbx_Model_OWK.Text, infFromBom[0], infFromBom[1], infFromBom[2], infFromBom[3], "", infFromBom[4], "", stl_nameUser.Text, rel_con);
                        }
                        havCodSm = true;
                    ...
                    else
                    {
                        ...
                        {
                            sttWK++;
                            DataRow dtrwWK = dtrans1WK.NewRow();
                            dtrwWK["STT"] = sttWK.ToString();
                            dtrwWK["Model"] = cbx_Model_OWK.Text;
                            dtrwWK["Mo_ta"] = infFromBom[0];
                            dtrwWK["Ma_NVL"] = infFromBom[1];
                            dtrwWK["Maker"] = infFromBom[2];
                            dtrwWK["Maker_Part"] = infFromBom[3];
                            dtrwWK["Lot"] = "";
                            dtrwWK["So_luong_nhap"] = infFromBom[4];
                            dtrwWK["Tem_code"] = "";
                            dtrwWK["IQC_test"] = "";
                            dtrans1WK.Rows.Add(dtrwWK);
                            dtrans1WK.AcceptChanges();
                        }                       
                        havCodSm = true;
                    }
                    ...
                    if (havCodSm == true)
                    {
                        txtQtRow.Text = "";
                        //Auto scroll
                        if (dgv_WH_Ktz.RowCount > 22)
                        {
                            dgv_WH_Ktz.FirstDisplayedScrollingRowIndex = dgv_WH_Ktz.RowCount - 1;
                        }
                        //Not sort
                        ...
                        picBx1.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma1 + ".PNG");
                        picBx1.SizeMode = PictureBoxSizeMode.StretchImage;
                        ...
                        if (strIma2 != string.Empty)
                        {
                            ...
                            picBx2.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma2 + ".PNG");
                            picBx2.SizeMode = PictureBoxSizeMode.StretchImage;
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    MessageBox.Show("NVL đang input không trùng Maker Part với NVL đã input trước đó.\nHãy hoàn thành in tem code với NVL hiện tại!", "OrderWH", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    chkMkp_WK = true;
                }                
            }
        }

        private void chkb_more1_CheckedChanged(object sender, EventArgs e)
        {
            if(chkb_more1.Checked == true)
            {
                if (arrgPerWK == true)
                ...
                else
                {
                    ...
                    if (Isopen == false)
                    {
                        InputMore confirmMr = new InputMore(this);
                        confirmMr.Show();
                        count_timer = 0;
                        timer1.Start();
                    }
            ...
            if (rbtnReload.Checked == true)
            {
                if(dgv_WH_Ktz.RowCount == 0)
                {
                    ...
                    try
                    {
                        ...
                        dtrans1WK = new DataTable();
                        string strPath = string.Empty;
                        strPath = @Application.StartupPath + "\\tem\\" + cbx_Model_OWK.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadWH-KTZ.txt";

                        StreamReader sr = new StreamReader(strPath);
                        string[] colName = sr.ReadLine().Split(',');
                        for (int j = 0; j < colName.Length - 1; j++)
                        {
                            dtrans1WK.Columns.Add(colName[j]);
                        }

                        string newLine;
                        while ((newLine = sr.ReadLine()) != null)
                        {
                            DataRow dtr = dtrans1WK.NewRow();
                ...
                {
                    MessageBox.Show("Bạn không thể tải dữ liệu lúc này!", "OrderWH", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    rbtnReload.Checked = false;
                }               
            }
        }
        ...
        private void btn_ConfirmWHKtz_Click(object sender, EventArgs e)
        {
            ...
            if (Isopen == false)
            {
                NewCode newCode = new NewCode(dgv_WH_Ktz, txt_user.Text, txt_pass.Text, cbx_Model_OWK.Text, stl_nameUser.Text, part, dMon, datTim, this);
                newCode.Show();
            }            
            //Reset data
            rbtnReload.Checked = false;
        ...
        {
            Clipboard.Clear();
            if ((DateTime.Compare(DateTime.Parse(dTiPic1WK.Value.ToShortDateString()), DateTime.Parse(dTiPic2WK.Value.ToShortDateString())) > 0) || (cbx_Model_OWK.Text.Length == 0))
            {
            ...
            else
            {
                ...
                int num_filOk = 0;
                int num_file = dtb1.count_file(@Application.StartupPath + "\\History\\WH\\Order\\", hisroryCheck);
                string[] nam_file = dtb1.get_filOK(num_file, @Application.StartupPath + "\\History\\WH\\Order\\", hisroryCheck, cbx_Model_OWK.Text, dTiPic1WK.Text, dTiPic2WK.Text, num_filOk);
                //Sprire.XLS -> merge all file 
                //Open file merged(save tạm ra đâu đó)
                //Save as file merged nếu muốn
                dtb1.merg_Excel(@Application.StartupPath + "\\History\\WH\\Order\\", hisroryCheck, nam_file, nam_file.Length, datTim, "WH-KTZ", true);
            } 
        }
        ...
        private void dgv_WH_Ktz_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (column == 9 && dgv_WH_Ktz.CurrentRow.Cells["IQC_test"].Value.ToString() != "")
            {
            ...
            else if (column == 9 && dgv_WH_Ktz.CurrentRow.Cells["IQC_test"].Value.ToString() == "")
            {
                dgv_WH_Ktz.CurrentRow.Cells["So_luong_nhap"].Value = (qtyInRoll).ToString();
            }
        }
        ...
        private void dgv_WH_Ktz_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            Thread.Sleep(100);
            int column = dgv_WH_Ktz.CurrentCell.ColumnIndex;
            if (column == 6 && dgv_WH_Ktz.CurrentRow.Cells["Lot"].Value.ToString() != "")
            {
        ...
        private void lbl_Lot1_Click(object sender, EventArgs e)
        {
            if (arrgPerWK == true)//la admin or manager
            {
                ...
                if (Isopen == false)
                {
                    InformationPicture infPicture = new InformationPicture(arrgPerWK);
                    infPicture.Show();
                }
            }
        }
        ...
        private void btn_enterKp_Click(object sender, EventArgs e)
        {
            if (txt_autoInputKp.Text == "")
            {
                ...
                {
                    txt_manualInputKp.Text = crtUpperChar(txt_manualInputKp.Text);
                    //Check double Input
        ...
        public void input_KP(TextBox txt, bool chkFifo)
        {
            ...
            if (minTimInp == "false")
            {
                ...
                else
                {
                    ...
                    {
                        txt.Text = "";
                        dtb1.del_filLog("PDxacnhan", str_Inp[1], 1);                        
                    }
                    chkScCode_KP = true;
                }
            }
            ...
            else
            {
                ...
                if (infFromBom[0] != null && infFromBom[1] != null && infFromBom[2] != null && infFromBom[3] != null && infFromBom[4] != null)
                {
                    ...
                    //Check clear lot
                    if (dtb1.checkClearLot(txt.Text) == true)
                    {
                        ...
                        if (dgv_Ktz_Pd.Columns.Count == 0 || dgv_Ktz_Pd.Rows.Count == 0)
                        {
                            ...
                            if (mkpSam1 == true)
                            {
                                dtb1.del_filLog("PDxacnhan", txt.Text, 1);
                                txt.Text = "";
                        ...
                        if (havCodSm1 == true)
                        {
                            ...
                            picBx1KP.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma1 + ".PNG");
                            picBx1KP.SizeMode = PictureBoxSizeMode.StretchImage;
                            ...
                            if (strIma2 != string.Empty)
                            {
                                ...
                                picBx2KP.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma2 + ".PNG");
                                picBx2KP.SizeMode = PictureBoxSizeMode.StretchImage;
                            }
                            else
                            {
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("Bạn chưa xuất hết Lot theo ngày. Hãy kiểm tra lại!", "In/Out Material", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt.Focus();//Trỏ chuột tại textBox Input
                    }                   
                }
            }
        }
        ...
        private void cb_inputCodeKp_CheckedChanged(object sender, EventArgs e)
        {
            if (cb_inputCodeKp.Checked == true)
            {
                ...
                else
                {
                    ...
                    if (Isopen == false)
                    {
                        ConfirmInOut confirmAd = new ConfirmInOut(this);
                        confirmAd.Show();
                        count_timer = 0;
                        timer1.Start();
                    }
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (this.dgv_Ktz_Pd.CurrentRow.Index >= 0)
                {
                    ...
                    dtb1.del_filLog("PDxacnhan", dgv_Ktz_Pd.Rows[dgv_Ktz_Pd.CurrentRow.Index].Cells["Tem_code"].Value.ToString(), 1);
                    dtb1.del_filLog("MakerPart", dgv_Ktz_Pd.Rows[dgv_Ktz_Pd.CurrentRow.Index].Cells["Maker_Part"].Value.ToString(), 1);
        ...
        private void btn_ConfirmKtz_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    //Xoa Stock FIFO
                    if (dtb1.del_FIFO(dgv_Ktz_Pd) == true)
                    {
                        DataTable dtSource = (DataTable)dgv_Ktz_Pd.DataSource;
                        //Giảm stock sau khi KTZ giao NVL cho PD
                        string[] err = dtb1.Reduce_StokKtz2(dgv_Ktz_Pd, datTim, "Ma_NVL", "Lot", "So_luong_cap", "In/Out Material", dtSource, stl_nameUser.Text, "KTZ-PD", dDay, dShift, cbx_ModelKP.Text, cbx_PD.Text);
                        if (err[0] == null)
                        {
                            dgv_Ktz_Pd.Columns.Clear();
                            ...
                            tb_Excel = dtb1.LoadDatabase1("PDxacnhanStock", cbx_ModelKP.Text, dDay);
                            string content = "Ngay_thang, Ca_kip, Line, Model, Mo_ta, Ma_NVL, Maker, Maker Part, Lot, So_luong_cap, Tem_code, KTZ, PD\n";
                            bool chekExitFil = excel.checkExitLog(@Application.StartupPath + "\\History\\In_Out\\Ktz_to_PD\\" + dMon + "\\" + datTim + "_" + cbx_ModelKP.Text + ".csv");
                            if (excel.Export_CSV(tb_Excel, @Application.StartupPath + "\\History\\In_Out\\Ktz_to_PD\\" + dMon + "\\" + datTim + "_" + cbx_ModelKP.Text + ".csv", chekExitFil, content) == true)
                            //if(excel.exportLogfileKP(dgv_Ktz_Pd, @Application.StartupPath + "\\History\\In_Out\\Ktz_to_PD\\" + dMon + "\\" + datTim + "_" + cbx_ModelKP.Text + ".csv", chekExitFil, 2, 1) == true)                           
                            {
                                MessageBox.Show("Tạo LogFile thành công!", "In/Out Material", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                if (File.Exists(@Application.StartupPath + "\\tem\\" + cbx_ModelKP.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadKTZ-PD.txt"))
                                {
                                    File.Delete(@Application.StartupPath + "\\tem\\" + cbx_ModelKP.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadKTZ-PD.txt");
                                }
                                ...
                                try
                                {
                                    string[] files = Directory.GetFiles(@Application.StartupPath + "\\Log\\Duplicate\\");
                                    int m = 0;
        ...
        private void btnStockLinee_Click(object sender, EventArgs e)
        {
            ...
            if (Isopen == false)
            {
                StockLine2 s_line2 = new StockLine2();
                s_line2.Show();
            }
        }
        ...
        private void radbtn_reLoadKP_CheckedChanged(object sender, EventArgs e)
        {
            if (radbtn_reLoadKP.Checked == true)
            {
                if(dgv_Ktz_Pd.RowCount == 0)
                {
                    ...
                    {
                        //DataTable dt_reload = new DataTable();
                        transport1KP = new DataTable();
                        StreamReader sr = new StreamReader(@Application.StartupPath + "\\tem\\" + cbx_ModelKP.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadKTZ-PD.txt");
                        string[] colName = sr.ReadLine().Split(',');
                        for (int j = 0; j < colName.Length - 1; j++)
                        {
                            transport1KP.Columns.Add(colName[j]);
                        }

                        string newLine;
                        while ((newLine = sr.ReadLine()) != null)
                        {
                            DataRow dtr = transport1KP.NewRow();
                            string[] values = newLine.Split(',');
                            if (values[0] != "")
                            {
                                for (int i = 0; i < values.Length - 1; i++)
        ...
        {
            if ((DateTime.Compare(DateTime.Parse(dTKP_Pic1.Value.ToShortDateString()), DateTime.Parse(dTKP_Pic2.Value.ToShortDateString())) > 0) || (cbx_ModelKP.Text.Length == 0))
            {
            ...
            else
            {
                ...
                //Get file name trong khoảng time đã chọn
                int num_filOk = 0;
                int num_file = dtb1.count_file(@Application.StartupPath + "\\History\\In_Out\\Ktz_to_PD\\", hisroryCheck);
                string[] nam_file = dtb1.get_filOK(num_file, @Application.StartupPath + "\\History\\In_Out\\Ktz_to_PD\\", hisroryCheck, cbx_ModelKP.Text, dTKP_Pic1.Text, dTKP_Pic2.Text, num_filOk);
                //Sprire.XLS -> merge all file 
                //Open file merged(save tạm ra đâu đó)
                //Save as file merged nếu muốn
                dtb1.merg_Excel(@Application.StartupPath + "\\History\\In_Out\\Ktz_to_PD\\", hisroryCheck, nam_file, nam_file.Length, datTim, "KTZ-PD", false);     
            }         
        }
        ...
        private void lblKP_Pic1_Click(object sender, EventArgs e)
        {
            if (arrgPerKP == true)//la admin or manager
            {
                ...
                if (Isopen == false)
                {
                    InformationPicture infPicture = new InformationPicture(arrgPerKP);
                    infPicture.Show();
                }
            }
        }
        ...
        private void dgv_Ktz_Pd_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (column == 10 && dgv_Ktz_Pd.CurrentRow.Cells["So_luong_cap"].Value.ToString() != "")
            {
                ...
                else
                {
                    ...
                    //    btn_ConfirmKtz.Enabled = false;
                    //}
                    if (tt_code != int.Parse(dgv_Ktz_Pd.CurrentRow.Cells["So_luong_cap"].Value.ToString()))
                    {
        ...
        public void PDxacnhan(TextBox txt)
        {
            ...
            else if (dtb1.chekNewCodeInputed(txt_scnCodePDxn.Text) == false)//da input r
            {
                txt.Text = "";
                txt.Enabled = false;
            ...
            else if(dtb1.chekdoubleCodePDxacnhan(txt_scnCodePDxn.Text) == true)
            {
                txt.Text = "";
                txt.Enabled = false;
            ...
            else if (dtb1.chekNVLHolding(txt_scnCodePDxn.Text) == false)
            {
                txt.Text = "";
                txt.Enabled = false;
            ...
            else
            {
                ...
                if (infFromBom[0] != null && infFromBom[1] != null && infFromBom[2] != null && infFromBom[3] != null && infFromBom[4] != null)
                {
                    if (nCode[0] == infFromBom[1] && txt.Text.Contains(nCode[2]))//trùng code và maker part
                    {
                        ...
                        //Luu new code
                        dtb1.savNwCodInputed(txt_scnCodePDxn.Text);
                        //Hinh ảnh
                        string strIma1 = string.Empty;
                        string strIma2 = string.Empty;
                        ...
                        picPDxn1.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma1 + ".PNG");
                        picPDxn1.SizeMode = PictureBoxSizeMode.StretchImage;
                        ...
                        if (strIma2 != string.Empty)
                        {
                            ...
                            picPDxn2.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma2 + ".PNG");
                            picPDxn2.SizeMode = PictureBoxSizeMode.StretchImage;
                        }
                        else
                        {
        ...
        private void rtb_reloadPDxn_CheckedChanged(object sender, EventArgs e)
        {
            if (rtb_reloadPDxn.Checked == true)
            {
                ...
                {
                    DataTable dt_reload = new DataTable();
                    StreamReader sr = new StreamReader(@Application.StartupPath + "\\tem\\" + cbx_modelPDxn.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadPDxacnhan.txt");
                    string[] colName = sr.ReadLine().Split(',');
                    for (int j = 0; j < colName.Length - 1; j++)
                    {
                    ...
                    dgv_PDxn.Columns.Clear();
                    dtb1.show_PDxacnhan(dgv_PDxn, dt_reload);
                    //An bot data
                    dgv_PDxn.Columns["Ngay_thang"].Visible = false;
                    dgv_PDxn.Columns["Ca_kip"].Visible = false;
                    dgv_PDxn.Columns["Line"].Visible = false;
        ...
        private void btn_delPDxn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    dtb1.del_filLog("Input_Line", dgv_PDxn.Rows[dgv_PDxn.CurrentRow.Index].Cells["Tem_code"].Value.ToString(), 1);
                ...
                if (CheckData(dgv_PDxn) == true)
                {
                    if(betWen > 0)
                    {
                        ...
                        if(rell == DialogResult.Yes)
                        {
                            ...
                            {
                                bool chekExitFil = excel.checkExitLog(@Application.StartupPath + "\\History\\In_Out\\PD_xacnhan\\" + dMon + "\\" + datTim + "_" + cbx_modelPDxn.Text + ".csv");
                                if (excel.exportLogfilePDxn(dgv_PDxn, @Application.StartupPath + "\\History\\In_Out\\PD_xacnhan\\" + dMon + "\\" + datTim + "_" + cbx_modelPDxn.Text + ".csv", chekExitFil, 2, 1) == true)
                                {
                                    MessageBox.Show("Tạo LogFile thành công!", "PDxacnhan", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    if (File.Exists(@Application.StartupPath + "\\tem\\" + cbx_modelPDxn.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadPDxacnhan.txt"))
                                    {
                                        File.Delete(@Application.StartupPath + "\\tem\\" + cbx_modelPDxn.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadPDxacnhan.txt");
                                    }
                                    //update lich su
                                    #region
                                    try
                                            ...
                                            foreach (string fil in files)
                                            {
                                                ...
                                                else if (files[m].Contains("PDxacnhan"))
                                                {
                                                    foreach (DataGridViewRow dgr in dgv_PDxn.Rows)
                                                    {
                                                        if (dgr.Cells["Mo_ta"].Value != null && dgr.Cells["Mo_ta"].Value.ToString() != "")
                                                        {
                                                            dtb1.del_filLog("PDxacnhan", dgr.Cells["Tem_code"].Value.ToString(), 1);
                                                        }
                                                    }
                                                    dgv_PDxn.Columns.Clear();
                                                    goto jumpm;
        ...
        public string[,] arrNVL = new string[1000, 15];//1000 dong=1000 cuộn liệu, 13 cot 
        public bool CheckData(DataGridView dgv)
        {
            int rw = 0;
            //Get data NVL KTZ da giao
            DirectoryInfo dir = new DirectoryInfo(@Application.StartupPath + "\\History\\In_Out\\Ktz_to_PD\\" + dMon);
            if (dir.Exists)
            {                
                foreach (FileInfo fIn in dir.GetFiles())
                {
                    ...
                    {
                        StreamReader srSource = new StreamReader(fIn.DirectoryName + "\\" + fIn);
        ...
        public int GapPDKTZ(DataGridView dgv)
        {
            ...
            try
            {
                DirectoryInfo dir2 = new DirectoryInfo(@Application.StartupPath + "\\History\\In_Out\\PD_xacnhan\\" + dMon);
                if (dir2.Exists)
                {
        ...
        {
            if ((DateTime.Compare(dTiPic1_PDxn.Value, dTiPic2_PDxn.Value) > 0) || (cbx_modelPDxn.Text.Length == 0))
            {
            ...
            else
            {
                ...
                //Get file name trong khoảng time đã chọn
                int num_filOk = 0;
                int num_file = dtb1.count_file(@Application.StartupPath + "\\History\\In_Out\\PD_xacnhan\\", hisroryCheck);
                string[] nam_file = dtb1.get_filOK(num_file, @Application.StartupPath + "\\History\\In_Out\\PD_xacnhan\\", hisroryCheck, cbx_modelPDxn.Text, dTiPic1_PDxn.Text, dTiPic2_PDxn.Text, num_filOk);
                //Sprire.XLS -> merge all file 
                //Open file merged(save tạm ra đâu đó)
                //Save as file merged nếu muốn
                dtb1.merg_Excel(@Application.StartupPath + "\\History\\In_Out\\PD_xacnhan\\", hisroryCheck, nam_file, nam_file.Length, datTim, "PDxacnhan", false);    
            }
        }
        ...
        private void lbl_PDxn1_Click(object sender, EventArgs e)
        {
            if (arrgPerPDxn == true)//la admin or manager
            {
                ...
                if (Isopen == false)
                {
                    InformationPicture infPicture = new InformationPicture(arrgPerPDxn);
                    infPicture.Show();
                }
            }
        }
        ...
        private async void txt_autoInputPk_TextChanged(object sender, EventArgs e)
        {
            ...
            if (txt_autoInputPk.Text != "" && chkScCode_PK == true)
            {
                ...
                else
                {
                    txt_manualInputPk.Text = "";
                    //Check double Input
                    if (dtb1.chekdoubleCode1(txt_autoInputPk.Text) == false)
                    {
                    ...
                    //Check code cho PD xac nhan
                    else if (dtb1.chekdoubleCodePDxacnhan(txt_autoInputPk.Text) == false)
                    {
                        txt_autoInputPk.ResetText();
                    ...
                    else if (dtb1.chekNewCodeInputed(txt_autoInputPk.Text) == true)
                    {
                        txt_autoInputPk.ResetText();
        ...
        private void btn_EnterPK_Click(object sender, EventArgs e)
        {
            if (txt_autoInputPk.Text == "")
            {
                ...
                {
                    //Check double Input
                    if (dtb1.chekdoubleCode1(txt_manualInputPk.Text) == false)
                    {
                    ...
                    //Check code cho PD xac nhan
                    else if (dtb1.chekdoubleCodePDxacnhan(txt_manualInputPk.Text) == false)
                    {
                        //txt_manualInputPk.ResetText();
                        MessageBox.Show("Cuộn liệu đang chờ PD xác nhận. Hãy kiểm tra lại!", "In/Out Material", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt_manualInputPk.Focus();//Trỏ chuột tại textBox Input
                    ...
                    //check code da dc input line va chua return ve Ktz
                    else if (dtb1.chekNewCodeInputed(txt_manualInputPk.Text) == true)
                    {
                        //txt_manualInputPk.ResetText();
                        MessageBox.Show("NVL chưa được input Line, không thể trả về KTZ. Hãy kiểm tra lại!", "In/Out Material", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt_manualInputPk.Focus();//Trỏ chuột tại textBox Input
        ...
        public void input_PK(TextBox txt)
        {
            ...
            if (infFromBom[0] != null && infFromBom[1] != null && infFromBom[2] != null && infFromBom[3] != null && infFromBom[4] != null)
            {
                ...
                {   
                    ...
                    picBx1PK.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma1 + ".PNG");
                    picBx1PK.SizeMode = PictureBoxSizeMode.StretchImage;
                    ...
                    if (strIma2 != string.Empty)
                    {
                        ...
                        picBx2PK.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma2 + ".PNG");
                        picBx2PK.SizeMode = PictureBoxSizeMode.StretchImage;
                    }
                    else
                    {
        ...
        private void btn_Clear_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (this.dgv_Pd_Ktz.CurrentRow.Index >= 0)
                {
                    ...
                    dtb1.del_filLog("MakerPart", dgv_Pd_Ktz.Rows[dgv_Pd_Ktz.CurrentRow.Index].Cells["Maker_Part"].Value.ToString(), 1);
                ...
                if (traloi == DialogResult.Yes)
                {
                    ...
                    if (dtb1.upStokKtz2(dgv_Pd_Ktz, datTim) == true)
                    {
                        ...
                        if (dtb1.up_FIFO2(dgv_Pd_Ktz) == true)
                        {
                            ...
                            {
                                //Tạo logfile                             
                                bool chekExitFil = excel.checkExitLog(@Application.StartupPath + "\\History\\In_Out\\PD_return_Ktz\\" + dMon + "\\" + datTim + "_" + cbx_ModelPK.Text + ".csv");
                                if (excel.exportLogfile(dgv_Pd_Ktz, @Application.StartupPath + "\\History\\In_Out\\PD_return_Ktz\\" + dMon + "\\" + datTim + "_" + cbx_ModelPK.Text + ".csv", chekExitFil, 2, 1) == true)
                                {
                                    MessageBox.Show("Tạo LogFile thành công!", "In/Out Material", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    if (File.Exists(@Application.StartupPath + "\\tem\\" + cbx_ModelPK.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadPD-KTZ.txt"))
                                    {
                                        File.Delete(@Application.StartupPath + "\\tem\\" + cbx_ModelPK.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadPD-KTZ.txt");
                                    }
                                    ...
                                    try
                                    {
                                        ...
                                        for (int h = 0; h < dgv_Pd_Ktz.RowCount; h++)
                                        {
                                            ...
                                            {                                                
                                                ...
                                                if (int.Parse(dgv_Pd_Ktz.Rows[h].Cells["Slg_tra_KTZ"].Value.ToString()) > 0)//stock ktz > 0 -> stock line = 0
                                                {
                                                    ...
                                                    if(nvlHolding == true)
                                                    {
                                                        dtb1.savHolding(dgv_Pd_Ktz.Rows[h].Cells["Tem_code"].Value.ToString());
                                                    }
                                                    ...
                                                    dtb1.savInput1(dgv_Pd_Ktz.Rows[h].Cells["Tem_code"].Value.ToString());
                                                    //xoa code trong input_Line
                                                    dtb1.del_filLog("Input_Line", dgv_Pd_Ktz.Rows[h].Cells["Tem_code"].Value.ToString(), 1);
                                                }
                                        ...
                                        #endregion
                                        //Hien thi stock line
                                        dtb1.delete_Transport("KtzGiaoPd1");
                                        ...
                                        sttPK = 0;
                                        timer_reLoad.Stop();
                                        File.Delete(@Application.StartupPath + "\\Log\\Duplicate\\MakerPart.log");
                                        //Xoa database
                                        dtb1.delete_Transport("Pd_Ktz_tranfer");
                                        dtb1.delete_Transport("Pd_Ktz");
                                        //Xoa stock = 0
        ...
        private void radbtn_reLoadPK_CheckedChanged(object sender, EventArgs e)
        {
            if (radbtn_reLoadPK.Checked == true)
            {
                ...
                {
                    DataTable dt_reload = new DataTable();
                    StreamReader sr = new StreamReader(@Application.StartupPath + "\\tem\\" + cbx_ModelPK.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadPD-KTZ.txt");
                    string[] colName = sr.ReadLine().Split(',');
                    for (int j = 0; j < colName.Length - 1; j++)
                    {
                        dt_reload.Columns.Add(colName[j]);
                    ...
                    dgv_Pd_Ktz.Columns.Clear();
                    dtb1.show_Pd_Ktz(dgv_Pd_Ktz, dt_reload);
                    //An bot datagridview
                    dgv_Pd_Ktz.Columns["Ngay_thang"].Visible = false;
                    dgv_Pd_Ktz.Columns["Ca_kip"].Visible = false;
                    dgv_Pd_Ktz.Columns["Line"].Visible = false;
        ...
        {
            if ((DateTime.Compare(dTPK_Pic1.Value, dTPK_Pic2.Value) > 0) || (cbx_ModelPK.Text.Length == 0))
            {
                MessageBox.Show("Hãy xem lại Model/thời gian bạn muốn kiểm tra!", "In/Out Material", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                ...
                //Get file name trong khoảng time đã chọn
                int num_filOk = 0;
                int num_file = dtb1.count_file(@Application.StartupPath + "\\History\\In_Out\\PD_return_Ktz\\", hisroryCheck);
                string[] nam_file = dtb1.get_filOK(num_file, @Application.StartupPath + "\\History\\In_Out\\PD_return_Ktz\\", hisroryCheck, cbx_ModelPK.Text, dTPK_Pic1.Text, dTPK_Pic2.Text, num_filOk);
                //Sprire.XLS -> merge all file 
                //Open file merged(save tạm ra đâu đó)
                //Save as file merged nếu muốn
                dtb1.merg_Excel(@Application.StartupPath + "\\History\\In_Out\\PD_return_Ktz\\", hisroryCheck, nam_file, nam_file.Length, datTim, "PD-KTZ", false);
            }      
        }
        ...
        private void lblPK_Pic1_Click(object sender, EventArgs e)
        {
            if (arrgPerPK == true)//la admin or manager
            {
                ...
                if (Isopen == false)
                {
                    InformationPicture infPicture = new InformationPicture(arrgPerPK);
                    infPicture.Show();
                }
            }
        }
        ...
        private void dgv_Pd_Ktz_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if ((column == 11 && dgv_Pd_Ktz.CurrentRow.Cells["Slg_ton_Line"].Value.ToString() != ""))
            {
                int tt_qty = 0;
                //tinh stock
                if(chkb_NVLchuatra.Checked == false)
                ...
                else if((slgKTZ > 0 && slgLine > 0))
                {
                    ...
                    dgv_Pd_Ktz.CurrentRow.Cells["Slg_ton_Line"].Style.BackColor = Color.Red;
                    btn_Confirm.Enabled = false;
                    MessageBox.Show("Slg_tra_KTZ và Slg_ton_Line không đồng thới > 0!", "In/Out Material", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    dgv_Pd_Ktz.CurrentRow.Cells["Slg_tra_KTZ"].Style.BackColor = Color.White;
        ...
        private void btn_enterCodeKw_Click(object sender, EventArgs e)
        {
            if (txt_autoInputKw.Text == "")
            {
                ...
                {
                    txt_manualInputKw.Text = crtUpperChar(txt_manualInputKw.Text);
        ...
        public void inp_Re(TextBox txt)
        {
            ...
            if (infFromBom[0] != null && infFromBom[1] != null && infFromBom[2] != null && infFromBom[3] != null && infFromBom[4] != null)
            {
                ...
                if (havCodSmrl == true)
                {
                    ...
                    picBoxRe1.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma1 + ".PNG");
                    picBoxRe1.SizeMode = PictureBoxSizeMode.StretchImage;
                    ...
                    if (strIma2 != string.Empty)
                    {
                        ...
                        picBoxRe2.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma2 + ".PNG");
                        picBoxRe2.SizeMode = PictureBoxSizeMode.StretchImage;
                    }
                    else
                    {
        ...
        private void btn_xoaRe_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (this.dgv_returnWH.CurrentRow.Index >= 0)
                {
                    ...
                    dtb1.del_filLog("MakerPart", dgv_returnWH.Rows[dgv_returnWH.CurrentRow.Index].Cells["Maker_Part"].Value.ToString(), 1);
        ...
        private void btn_ConfRe_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    //Xoa Stock FIFO
                    if (dtb1.del_FIFO(dgv_returnWH) == true)
                    {
                        DataTable dtSource = (DataTable)dgv_returnWH.DataSource;
                        //Giảm stock sau khi return WH
                        string[] err = dtb1.Reduce_StokKtz2(dgv_returnWH, datTim, "Ma_NVL", "Lot", "So_luong_tra", "ReturnWH", dtSource, stl_nameUser.Text, "KTZ-WH", dDay, dShift, cbx_reMol.Text, "");
                        if (err[0] == null)
                        {
                            dgv_returnWH.Columns.Clear();
                            ...
                            bool chekExitFil = excel.checkExitLog(@Application.StartupPath + "\\History\\WH\\Return\\" + dMon + "\\" + datTim + "_" + cbx_reMol.Text + ".csv");
                            if (excel.Export_CSV(tb_Excel, @Application.StartupPath + "\\History\\WH\\Return\\" + dMon + "\\" + datTim + "_" + cbx_reMol.Text + ".csv", chekExitFil, content) == true)
                            {
                                MessageBox.Show("Tạo LogFile thành công!", "ReturnWH", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                if (File.Exists(@Application.StartupPath + "\\tem\\" + cbx_reMol.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadReturnWH.txt"))
                                {
                                    File.Delete(@Application.StartupPath + "\\tem\\" + cbx_reMol.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadReturnWH.txt");
                                }
                                ...
                                try
                                {
                                    ...
                                    foreach (string fil in files)
                                    {
                                        if (files[n].Contains("Input_Line") || files[n].Contains("PDxacnhan"))
                                        {
                                            goto jumpn;
                                        }
                                        else if (files[n].Contains("Input_Ktz"))
                                        {
                                            foreach (DataGridViewRow dgr in dgv_returnWH.Rows)
                                            {
                                                if (dgr.Cells["Mo_ta"].Value != null && dgr.Cells["Mo_ta"].Value.ToString() != "")
                                                {
                                                    dtb1.del_filLog("Input_Ktz", dgr.Cells["Tem_code"].Value.ToString(), 1);
                                                }
                                            }                                            
                                            goto jumpn;
                                        }
                                        else if(files[n].Contains("NVL_Holding"))
                                        {
                                            foreach (DataGridViewRow dgr in dgv_returnWH.Rows)
                                            {
                                                if (dgr.Cells["Mo_ta"].Value != null && dgr.Cells["Mo_ta"].Value.ToString() != "")
                                                {
                                                    dtb1.del_filLog("NVL_Holding", dgr.Cells["Tem_code"].Value.ToString(), 1);
                                                }
                                            }                                           
                                            goto jumpn;
                                        }
        ...
        private void rdbtn_reloadRe_CheckedChanged(object sender, EventArgs e)
        {
            if (rdbtn_reloadRe.Checked == true)
            {
                if(dgv_returnWH.RowCount == 0)
                {
                    ...
                    {
                        //DataTable dt_reload = new DataTable();
                        reltran1KW = new DataTable();
                        StreamReader sr = new StreamReader(@Application.StartupPath + "\\tem\\" + cbx_reMol.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadReturnWH.txt");
                        string[] colName = sr.ReadLine().Split(',');
                        for (int j = 0; j < colName.Length - 1; j++)
                        {
                            reltran1KW.Columns.Add(colName[j]);
                        }

                        string newLine;
                        while ((newLine = sr.ReadLine()) != null)
                        {
                            DataRow dtr = reltran1KW.NewRow();
                            string[] values = newLine.Split(',');
                            if (values[0] != "")
                            {
                                for (int i = 0; i < values.Length - 1; i++)
                ...
                {
                    MessageBox.Show("Bạn không thể tải dữ liệu lúc này!", "ReturnWH", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    rdbtn_reloadRe.Checked = false;
                }               
            }
        }        
        ...
        {
            if ((DateTime.Compare(DateTime.Parse(daTiPic_KW1.Value.ToShortDateString()), DateTime.Parse(daTiPic_KW2.Value.ToShortDateString())) > 0) || (cbx_reMol.Text.Length == 0))
            {
            ...
            else
            {
                ...
                //Get file name trong khoảng time đã chọn
                int num_filOk = 0;
                int num_file = dtb1.count_file(@Application.StartupPath + "\\History\\WH\\Return\\", hisroryCheck);
                string[] nam_file = dtb1.get_filOK(num_file, @Application.StartupPath + "\\History\\WH\\Return\\", hisroryCheck, cbx_reMol.Text, daTiPic_KW1.Text, daTiPic_KW2.Text, num_filOk);
                //Sprire.XLS -> merge all file 
                //Open file merged(save tạm ra đâu đó)
                //Save as file merged nếu muốn
                dtb1.merg_Excel(@Application.StartupPath + "\\History\\WH\\Return\\", hisroryCheck, nam_file, nam_file.Length, datTim, "ReturnWH", true);
            }
        }
        ...
        private void lbl_picRe1_Click(object sender, EventArgs e)
        {
            if (arrgPerKW == true)//la admin or manager
            {
                ...
                if (Isopen == false)
                {
                    InformationPicture infPicture = new InformationPicture(arrgPerKW);
                    infPicture.Show();
                }
            }
        }
        ...
        private void dgv_returnWH_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if ((column == 6 && dgv_returnWH.CurrentRow.Cells["So_luong_tra"].Value.ToString() != ""))
            {
                ...
                {
                    string[] strFifo = dgv_returnWH.CurrentRow.Cells["Tem_code"].Value.ToString().Split('+');
                    int tt = dtb1.getData_qty2(dgv_returnWH.CurrentRow.Cells["Ma_NVL"].Value.ToString(), dgv_returnWH.CurrentRow.Cells["Maker_Part"].Value.ToString(), dgv_returnWH.CurrentRow.Cells["Lot"].Value.ToString(), strFifo[1]);
                    if (tt < int.Parse(dgv_returnWH.CurrentRow.Cells[6].Value.ToString()))
                    {
        ...
        //=============================================================KTZ-Other===============================================================================
        #region KTZ-Other
        private void cbx_modelKO_SelectedIndexChanged(object sender, EventArgs e)
        {
            dtb1.delete_Transport("KTZother");
            dgv_KO.Columns.Clear();
            txt_autoKO.Focus();
        }

        private void cbx_bpKO_SelectedIndexChanged(object sender, EventArgs e)
        {
            dtb1.get_cbbData("Login", "Name_user", "part", cbx_bpKO.Text, cbx_aiKO);
        }

        private void cbx_aiKO_SelectedIndexChanged(object sender, EventArgs e)
        {
            if(chkb_nhtayKO.Checked == true)
            {
                txt_manualKO.Focus();
            }
            else
        ...
        private void chkb_nhtayKO_CheckedChanged(object sender, EventArgs e)
        {
            if (chkb_nhtayKO.Checked == true)
            {
                AcceptButton = btn_enterKO;
                if (arrgPerKO == true)
                {
                    txt_manualKO.Visible = true;
                    txt_manualKO.Enabled = true;
            ...
            else
            {
                ...
                txt_manualKO.Text = "";
                txt_autoKO.Focus();
                btn_enterKO.Visible = false;
                btn_enterKO.Enabled = false;
                lbl_inpKO.Text = "Scan Code";
            }
        }

        private async void txt_autoKO_TextChanged(object sender, EventArgs e)
        {
            await Task.Delay(2000);
            if (txt_autoKO.Text != "" && chkScCode_KO == true)
            {
                chkScCode_KO = false;
                if (cbx_modelKO.Text == "" || cbx_bpKO.Text == "" || cbx_aiKO.Text == "")
                {
                    MessageBox.Show("Bạn điền thiếu thông tin Model/Bộ phận/PIC!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txt_autoKO.Text = "";
                    cbx_modelKO.Focus();
                    chkScCode_KO = true;
                }
                ...
                {
                    txt_manualKO.Text = "";
                    if (dtb1.chekdoubleCodeOther(txt_autoKO.Text) == false)//check double input
                    {
                        txt_autoKO.ResetText();
                        MessageBox.Show("Trùng cuộn liệu đã nhập trước đó. Hãy kiểm tra lại!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt_autoKO.Focus();//Trỏ chuột tại textBox Input
                        chkScCode_KO = true;
                    }
                    else if (dtb1.chk_formInput(txt_autoKO.Text) == false)//check format code
                    {
                        txt_autoKO.ResetText();
                        MessageBox.Show("Sai format code input. Hãy kiểm tra lại!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt_autoKO.Focus();//Trỏ chuột tại textBox Input
                        chkScCode_KO = true;
                    }
                    else if (dtb1.get_extinctCode(txt_autoKO.Text) == false)//check tồn tại trong stock KTZ4
                    {
                        txt_autoKO.ResetText();
                        MessageBox.Show("Không tồn tại code đã input. Hãy kiểm tra lại!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt_autoKO.Focus();//Trỏ chuột tại textBox Input
                        chkScCode_KO = true;
                    }
                    else
                    {
                        if (arrgPerKO == false)//OP
                        {
                            //Gọi hàm kiểm tra thông tin input
                            inp_KO(txt_autoKO);
                        }
                        ...
                        {
                            DialogResult rel_ar = MessageBox.Show("Bạn đang làm công việc của OP. Bạn có muốn tiếp tục?", "KTZother", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                            if (rel_ar == DialogResult.OK)
                            {
                                //Gọi hàm kiểm tra thông tin input
                                inp_KO(txt_autoKO);
        ...
        private void btn_enterKO_Click(object sender, EventArgs e)
        {
            if (txt_autoKO.Text == "")
            {
                if (cbx_modelKO.Text == "" || txt_manualKO.Text == "" || cbx_bpKO.Text == "" || cbx_aiKO.Text == "")
                {
                    MessageBox.Show("Hãy kiểm tra lại thông tin Input!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    cbx_modelKO.Focus();
                }
                else
                {
                    txt_manualKO.Text = crtUpperChar(txt_manualKO.Text);
                    if (dtb1.chekdoubleCodeOther(txt_manualKO.Text) == false)//check double input
                    {
                        MessageBox.Show("Trùng cuộn liệu đã nhập trước đó. Hãy kiểm tra lại!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt_manualKO.Focus();//Trỏ chuột tại textBox Input
                    }
                    else if (dtb1.chk_formInput(txt_manualKO.Text) == false)//check format code
                    {
                        MessageBox.Show("Sai format code input. Hãy kiểm tra lại!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt_manualKO.Focus();//Trỏ chuột tại textBox Input
                    }
                    else if (dtb1.get_extinctCode(txt_manualKO.Text) == false)//check tồn tại trong stock KTZ4
                    {
                        MessageBox.Show("Không tồn tại code đã input trong stock. Hãy kiểm tra lại!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txt_manualKO.Focus();//Trỏ chuột tại textBox Input
                    }
                    else
                    {
            ...
            {
                MessageBox.Show("Hãy xóa mục Scan Code trước khi Enter code tay!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }

        public void inp_KO(TextBox txt)
        {
            //Biến xác nhận maker part same
            bool mkpSmOt = false;
            //Biến báo đã có code trong DataGirdView trùng
            bool havCodSmOt = false;
            //tach chuoi và so sanh trong stock
            string[] str_Inp = txt.Text.Split('+');
            dtKtzOther = dtb1.LoadStockFIFO(str_Inp[0], str_Inp[2], str_Inp[3], str_Inp[1]);
            //Lọc từ BOM theo makerpart, nếu trùng all thông tin > 2 lần trong dataTable -> báo PE xác nhận                      
            string[] infFromBom = get_InfInpKW(dtKtzOther, txt, chkb_nhtayKO);
            if (infFromBom[0] != null && infFromBom[1] != null && infFromBom[2] != null && infFromBom[3] != null && infFromBom[4] != null)
            {
                //Kiểm tra maker part xem có dc scan >=2 lần              
                if (dtb1.chekScanMakPrtSame(infFromBom[3]) == false)//đã có input maker part rồi
                ...
                sttKO++;
                //Điền data vào datagridview
                if (dgv_KO.Columns.Count == 0 || dgv_KO.Rows.Count == 0)//dgv chua co data
                {
                    timer_reLoad.Start();
                    tool_saving.BackColor = Color.Green;
                    dtb1.delete_Transport("KTZother");
                    if (mkpSmOt == true)//da ton tai new makerpart
                    {
                        txt.Text = "";
                        MessageBox.Show("Kiểm tra lại file log (Makerpart) trong StartPath!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        chkScCode_KO = true;
                    }
                    else//input new makerpart
                    {
                        dtb1.insert_transOther(sttKO.ToString(), dDay, dShift, cbx_modelKO.Text, infFromBom[0], infFromBom[1], infFromBom[2], infFromBom[3], str_Inp[3], infFromBom[4], txt.Text, stl_nameUser.Text, cbx_aiKO.Text);
                        havCodSmOt = true;
                        dt1KtzOther = dtb1.loadtransport_tableWH("KTZother");
                        //Hiển thị
                        dgv_KO.Columns.Clear();
                        dtb1.show_KTZother(dgv_KO, dt1KtzOther);
                    }
                }
                else//dgv da co data
                {
                    tool_saving.BackColor = Color.Green;
                    DataRow dtrwKO = dt1KtzOther.NewRow();
                    dtrwKO["STT"] = sttKO.ToString();
                    dtrwKO["Ngay_thang"] = dDay;
                    dtrwKO["Ca_kip"] = dShift;
                    dtrwKO["Line"] = "SMD";
                    dtrwKO["Model"] = cbx_modelKO.Text;
                    dtrwKO["Mo_ta"] = infFromBom[0];
                    dtrwKO["Ma_NVL"] = infFromBom[1];
                    dtrwKO["Maker"] = infFromBom[2];
                    dtrwKO["Maker_Part"] = infFromBom[3];
                    dtrwKO["Lot"] = str_Inp[3];
                    dtrwKO["So_luong_tra"] = infFromBom[4];
                    dtrwKO["Tem_code"] = txt.Text;
                    dtrwKO["Giai_thich"] = "";
                    dtrwKO["Ghi_chu"] = "";
                    dtrwKO["KTZ"] = stl_nameUser.Text;
                    dtrwKO["Nguoi_order"] = cbx_aiKO.Text;
                    dt1KtzOther.Rows.Add(dtrwKO);
                    dt1KtzOther.AcceptChanges();
                    havCodSmOt = true;
                }
                ...
                {
                    //An bot datagridvuew 
                    dgv_KO.Columns["Ngay_thang"].Visible = false;
                    dgv_KO.Columns["Ca_kip"].Visible = false;
                    dgv_KO.Columns["Line"].Visible = false;
                    dgv_KO.Columns["Model"].Visible = false;
                    dgv_KO.Columns["KTZ"].Visible = false;
                    dgv_KO.Columns["Nguoi_order"].Visible = false;
                    //Auto scroll
                    if (dgv_KO.RowCount > 22)
                    {
                        dgv_KO.FirstDisplayedScrollingRowIndex = dgv_KO.RowCount - 1;
                    }
                    //Not sort
                    foreach (DataGridViewColumn col in dgv_KO.Columns)
                    {
                        col.SortMode = DataGridViewColumnSortMode.NotSortable;
                    }
                    //save data input
                    dtb1.savInput3(txt.Text);
                    //Save data FIFO                               
                    //dtb1.savFIFO(infFromBom[1] + "+" + infFromBom[2] + "+" + infFromBom[3] + "+" + str_Inp[3] + "+" + str_Inp[1]);
                    //hien thi label
                    string strIma1 = string.Empty;
                    ...
                    picBox_1KO.Visible = true;
                    lbl_pic1KO.Visible = true;

                    picBox_1KO.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma1 + ".PNG");
                    picBox_1KO.SizeMode = PictureBoxSizeMode.StretchImage;

                    if (strIma2 != string.Empty)
                    {
                        picBox_2KO.Visible = true;
                        lbl_pic2KO.Visible = true;
                        picBox_2KO.Image = new Bitmap(@Application.StartupPath + "\\Picture\\" + strIma2 + ".PNG");
                        picBox_2KO.SizeMode = PictureBoxSizeMode.StretchImage;
                    }
                    else
                    {
                        picBox_2KO.Visible = false;
                        lbl_pic2KO.Visible = false;
                    }
                    havCodSmOt = false;
                    txt.Text = "";
                    if (chkb_nhtayKO.Checked == false)
                    {
                        chkScCode_KO = true;
                    }
                }
        ...
        private void btn_delKO_Click(object sender, EventArgs e)
        {
            txt_autoKO.Focus();
            try
            {
                if (this.dgv_KO.CurrentRow.Index >= 0)
                {
                    string[] str = dgv_KO.Rows[dgv_KO.CurrentRow.Index].Cells["Tem_code"].Value.ToString().Split('+');
                    string strFifo = str[0] + "+" +
                                     dgv_KO.Rows[dgv_KO.CurrentRow.Index].Cells["Maker"].Value.ToString() + "+" +
                                     str[2] + "+" +
                                     str[3] + "+" +
                                     str[1];
                    //dtb1.del_filLog("FI-FO", strFifo, 1);
                    dtb1.del_filLog("KTZ_Other", dgv_KO.Rows[dgv_KO.CurrentRow.Index].Cells["Tem_code"].Value.ToString(), 1);
                    dtb1.del_filLog("MakerPart", dgv_KO.Rows[dgv_KO.CurrentRow.Index].Cells["Maker_Part"].Value.ToString(), 1);

                    DataRow drDel = dt1KtzOther.Rows[dgv_KO.CurrentRow.Index];
                    dt1KtzOther.Rows.Remove(drDel);
                }
                if (dgv_KO.RowCount == 0)
                {
                    chkScCode_KO = true;
                    sttKO = 0;
                }
            ...
            {
                MessageBox.Show("Click vào đầu hàng đó để xóa!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }

        private void btn_xnKO_Click(object sender, EventArgs e)
        {
            if (dtb1.checkKtz_other(dgv_KO) == true)//kiểm tra datagirdview dc điền đủ thông tin
            {
                MessageBox.Show("Các thông tin đang để trống hoặc sai!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                DialogResult anReWH = MessageBox.Show("Bạn muốn lưu thông tin đã nhập?", "KTZother", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (anReWH == DialogResult.Yes)
                {
                    //Xoa Stock FIFO
                    if (dtb1.del_FIFO(dgv_KO) == true)
                    {
                        DataTable dtSource = (DataTable)dgv_KO.DataSource;
                        //Giảm stock sau khi return WH
                        string[] err = dtb1.Reduce_StokKtz3(dgv_KO, datTim, "Ma_NVL", "Lot", "So_luong_cap", "KTZother", dtSource, stl_nameUser.Text, dDay, dShift, cbx_modelKO.Text, cbx_aiKO.Text);
                        if (err[0] == null)
                        {
                            dgv_KO.Columns.Clear();
                            dtb1.show_KTZother(dgv_KO, dtSource);
                            //Not sort
                            foreach (DataGridViewColumn col in dgv_KO.Columns)
                            {
                                col.SortMode = DataGridViewColumnSortMode.NotSortable;
                            ...
                            //DataTable trung gian lưu thông tin theo form c/s -> gán vào file .CSV
                            DataTable tb_Excel = new DataTable();
                            tb_Excel = dtb1.LoadDatabase("KTZother_Logfile");
                            string content = "Ngay_thang, Ca_kip, Line, Model, Mo_ta, Ma_NVL, Maker, Maker_Part, Lot, So_luong_cap, Tem_code, Giai_thich, Ghi_chu, KTZ, Nguoi_order\n";
                            bool chekExitFil = excel.checkExitLog(@Application.StartupPath + "\\History\\In_Out\\Ktz_other\\" + dMon + "\\" + datTim + "_" + cbx_modelKO.Text + ".csv");
                            if (excel.Export_CSV(tb_Excel, @Application.StartupPath + "\\History\\In_Out\\Ktz_other\\" + dMon + "\\" + datTim + "_" + cbx_modelKO.Text + ".csv", chekExitFil, content) == true)
                            {
                                MessageBox.Show("Tạo LogFile thành công!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                if (File.Exists(@Application.StartupPath + "\\tem\\" + cbx_modelKO.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadKtzOther.txt"))
                                {
                                    File.Delete(@Application.StartupPath + "\\tem\\" + cbx_modelKO.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadKtzOther.txt");
                                }
                                ...
                                tool_saving.BackColor = Color.White;
                                dtb1.delete_Transport("KTZother");
                                dtb1.delete_Transport("KTZother_Logfile");
                                //Xoa stock = 0
                                dtb1.Del_StockZero("Stock_KTZ", "So_luong");
                                //xoa file .log
                                try
                                {
                                    ...
                                    foreach (string fil in files)
                                    {
                                        if (files[n].Contains("Input_Line") || files[n].Contains("Input_Ktz") || files[n].Contains("PDxacnhan") || files[n].Contains("NVL_Holding") || files[n].Contains("KTZ_Other"))
                                        {
                                            goto jumpn;
                                        }                                        
                                        File.Delete(fil);
                                ...
                                {
                                    MessageBox.Show("Xảy ra lỗi xóa file .log (FI-FO, MakerPart)!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                }
                            }
                            else
                            {
                                MessageBox.Show("Xảy ra lỗi xuất logfile!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                        }
                        else
                        {
                            ...
                            if (err[0] == "error")
                            {
                                MessageBox.Show("Bạn chưa nhập data. Nội dung hiển thị trống!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                            else
                            {
                                foreach (DataGridViewRow dgr in dgv_KO.Rows)
                                {
                                    for (int i = 0; i < err.Length; )
                                    {
                                        if (dgr.Cells["Mo_ta"].Value != null && dgr.Cells["Mo_ta"].Value.ToString() != "")
                    ...
                    {
                        MessageBox.Show("Xảy ra lỗi Xóa Stock FI-FO!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
        }

        private void btn_stkKtzKO_Click(object sender, EventArgs e)
        {
            //Hiển thị new form
            bool Isopen = false;
            foreach (Form f in Application.OpenForms)
        ...
        private void rbtn_reloadKO_CheckedChanged(object sender, EventArgs e)
        {
            if (rbtn_reloadKO.Checked == true)
            {
                if (dgv_KO.RowCount == 0)
                {
                    timer_reLoad.Start();
                    ...
                    {
                        //DataTable dt_reload = new DataTable();
                        dt1KtzOther = new DataTable();
                        StreamReader sr = new StreamReader(@Application.StartupPath + "\\tem\\" + cbx_modelKO.Text + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadKtzOther.txt");
                        string[] colName = sr.ReadLine().Split(',');
                        for (int j = 0; j < colName.Length - 1; j++)
                        {
                            dt1KtzOther.Columns.Add(colName[j]);
                        }

                        string newLine;
                        while ((newLine = sr.ReadLine()) != null)
                        {
                            DataRow dtr = dt1KtzOther.NewRow();
                            string[] values = newLine.Split(',');
                            if (values[0] != "")
                            {
                                for (int i = 0; i < values.Length - 1; i++)
                    ...
                    {
                        MessageBox.Show("Data Re-load trống. Hãy tiếp tục thao tác!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        rbtn_reloadKO.Checked = false;
                    }
                }
                else
                {
                    MessageBox.Show("Bạn không thể tải dữ liệu lúc này!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    rbtn_reloadKO.Checked = false;
                }
            }
        }

        private void btn_chkKO_Click(object sender, EventArgs e)
        {
            if ((DateTime.Compare(DateTime.Parse(daTim_1KO.Value.ToShortDateString()), DateTime.Parse(daTim_2KO.Value.ToShortDateString())) > 0) || (cbx_modelKO.Text.Length == 0))
            {
                MessageBox.Show("Hãy xem lại Model/thời gian bạn muốn kiểm tra!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                string[] hisroryCheck = GetHistory(daTim_1KO, daTim_2KO);
                //Tìm history theo ngày -> tổng hợp -> Hiển thị Excel
                //Get file name trong khoảng time đã chọn
                int num_filOk = 0;
                int num_file = dtb1.count_file(@Application.StartupPath + "\\History\\In_Out\\Ktz_other\\", hisroryCheck);
                string[] nam_file = dtb1.get_filOK(num_file, @Application.StartupPath + "\\History\\In_Out\\Ktz_other\\", hisroryCheck, cbx_modelKO.Text, daTim_1KO.Text, daTim_2KO.Text, num_filOk);
                //Sprire.XLS -> merge all file 
                //Open file merged(save tạm ra đâu đó)
                //Save as file merged nếu muốn
                dtb1.merg_Excel(@Application.StartupPath + "\\History\\In_Out\\Ktz_other\\", hisroryCheck, nam_file, nam_file.Length, datTim, "KTZother", false);
            }
        }

        private void lbl_pic1KO_Click(object sender, EventArgs e)
        {
            if (arrgPerKO == true)//la admin or manager
            {
                bool Isopen = false;
                foreach (Form f in Application.OpenForms)
                {
                ...
                if (Isopen == false)
                {
                    InformationPicture infPicture = new InformationPicture(arrgPerKO);
                    infPicture.Show();
                }
            }
        }

        private void lbl_pic2KO_Click(object sender, EventArgs e)
        {
            if (arrgPerKO == true)//la admin or manager
            {
                bool Isopen = false;
        ...
        private void dgv_KO_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            Thread.Sleep(300);
            int column = dgv_KO.CurrentCell.ColumnIndex;
            if ((column == 10 && dgv_KO.CurrentRow.Cells["So_luong_cap"].Value.ToString() != ""))
            {
                int n = 0;
                bool chkgd = int.TryParse(dgv_KO.CurrentRow.Cells["So_luong_cap"].Value.ToString(), out n);
                if (chkgd == false || n < 0)
                {
                    MessageBox.Show("Mục So_luong_cap phải số dương. Không nhập chữ hoặc số âm!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    dgv_KO.CurrentRow.Cells["So_luong_cap"].Style.BackColor = Color.Red;
                    btn_xnKO.Enabled = false;
                }
                else
                {
                    string[] strFifo = dgv_KO.CurrentRow.Cells["Tem_code"].Value.ToString().Split('+');
                    int tt = dtb1.getData_qty2(dgv_KO.CurrentRow.Cells["Ma_NVL"].Value.ToString(), dgv_KO.CurrentRow.Cells["Maker_Part"].Value.ToString(), dgv_KO.CurrentRow.Cells["Lot"].Value.ToString(), strFifo[1]);
                    if (tt < int.Parse(dgv_KO.CurrentRow.Cells[10].Value.ToString()))
                    {
                        MessageBox.Show("NVL " + dgv_KO.CurrentRow.Cells["Ma_NVL"].Value.ToString() + ",Lot " + dgv_KO.CurrentRow.Cells["Lot"].Value.ToString() + " stock KTZ (" + tt.ToString() + ") < So_luong_cap (" + dgv_KO.CurrentRow.Cells["So_luong_cap"].Value.ToString() + ") \nHãy kiểm tra lại số lượng trả WH!", "KTZother", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        dgv_KO.CurrentRow.Cells["So_luong_cap"].Style.BackColor = Color.Red;
                        btn_xnKO.Enabled = false;
                    }                    
                    else
                    {
                        dgv_KO.CurrentRow.Cells["So_luong_cap"].Style.BackColor = Color.White;
                        btn_xnKO.Enabled = true;
                    }
                }
            }
            else if ((column == 12 && dgv_KO.CurrentRow.Cells["Giai_thich"].Value.ToString() != ""))
            {
                if (dgv_KO.CurrentRow.Cells["Giai_thich"].Value.ToString() == "Khác")
                {
                    dgv_KO.CurrentRow.Cells["Ghi_chu"].ReadOnly = false;
                    dgv_KO.CurrentCell = dgv_KO.CurrentRow.Cells["Giai_thich"];
                }
                else
                {
                    dgv_KO.CurrentRow.Cells["Ghi_chu"].Value = "";
                    dgv_KO.CurrentRow.Cells["Ghi_chu"].ReadOnly = true;
                }
            }
        }

        private void cbx_modelKO_MouseMove(object sender, MouseEventArgs e)
        {
            var x = posX;
            var y = posY;
        ...
        private void cbx_bpKO_MouseMove(object sender, MouseEventArgs e)
        {
            var x = posX;
            var y = posY;
        ...
        private void cbx_aiKO_MouseMove(object sender, MouseEventArgs e)
        {
            var x = posX;
            var y = posY;
        ...
        private void txt_autoKO_MouseMove(object sender, MouseEventArgs e)
        {
            var x = posX;
            var y = posY;
        ...
        private void txt_manualKO_MouseMove(object sender, MouseEventArgs e)
        {
            var x = posX;
            var y = posY;
        ...
        private void btn_delKO_MouseMove(object sender, MouseEventArgs e)
        {
            var x = posX;
            var y = posY;
        ...
        private void btn_xnKO_MouseMove(object sender, MouseEventArgs e)
        {
            var x = posX;
            var y = posY;
        ...
        private void btn_stkKtzKO_MouseMove(object sender, MouseEventArgs e)
        {
            var x = posX;
            var y = posY;
        ...
        private void btn_enterKO_MouseMove(object sender, MouseEventArgs e)
        {
            var x = posX;
            var y = posY;
        ...
        private void btn_chkKO_MouseMove(object sender, MouseEventArgs e)
        {
            var x = posX;
            var y = posY;
        ...
        private void dgv_KO_MouseMove(object sender, MouseEventArgs e)
        {
            var x = posX;
            var y = posY;
