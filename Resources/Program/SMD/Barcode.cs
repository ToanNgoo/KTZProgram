...
namespace ManageMaterialPBA
{
    public partial class Barcode : Form
    {
        ...
        public bool btnKTZ = false, btnLine = false, btnPDxn = false;
        ...
        public Barcode(string _user, string _pass)
        {
            InitializeComponent();
            userr = _user;
            passs = _pass;
        ...
        {
            if (chk_link.Checked == true && chk_codeSigl.Checked == true)
            {
                MessageBox.Show("Lựa chọn :\n1.In code theo link file\n2.Nhập tay code cần in\n3.Không chọn phương án nào\nKhông chọn đồng thời cả 2 phương án!", "In Code", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                chk_link.Checked = false;
                chk_codeSigl.Checked = false;
                label5.Text = "Code NVL";
                cbx_code.Visible = true;
                cbx_code.Enabled = true;
                txt_linkFile.Visible = false;
            ...
            {
                if (chk_link.Checked == true)
                {
                    incodLik();

                }
                else if (chk_codeSigl.Checked == true)
                {
                    inCodesgl();
                }
                else
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if(btnKTZ == true && cfrm == true)
            {
                try
                {
                    ...
                    cfrm = false;
                    timer1.Stop();
                    StreamReader sr = new StreamReader(@Application.StartupPath + "\\Print\\KTZ\\" + dP + "_NewCode.log");
                    while (sr.EndOfStream == false)
                    {
            ...
            if (btnLine == true && cfrm == true)
            {
                try
                {
                    ...
                    cfrm = false;
                    timer1.Stop();
                    StreamReader sr = new StreamReader(@Application.StartupPath + "\\Print\\Line\\" + dP + "_NewCode.log");
                    while (sr.EndOfStream == false)
                    {
                        string strP = sr.ReadLine();
                        cbx_code.Items.Add(strP);
            ...
            //In code tai stock PDxn
            if (btnPDxn == true && cfrm == true)
            {
                try
                {
                    count_timer = 0;
                    ...
                    cfrm = false;
                    timer1.Stop();
                    StreamReader sr = new StreamReader(@Application.StartupPath + "\\Print\\PDxn\\" + dP + "_NewCode.log");
                    while (sr.EndOfStream == false)
                    {
                        string strP = sr.ReadLine();
                        cbx_code.Items.Add(strP);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (Isopen == false)
            {
                StockKTZ stk = new StockKTZ(this);
                stk.Show();
                timer1.Start();
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (Isopen == false)
            {
                StockLine s_line = new StockLine(this);
                s_line.Show();
                timer1.Start();
            }
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (Isopen == false)
            {
                StockWaitPDxn s_PDxn = new StockWaitPDxn(this);
                s_PDxn.Show();
                timer1.Start();
            }
        }
        ...
        public void inCodesgl()
        {
            ...
            if (chkSetup == true)
            {
                if (dtb1.chk_formInput(txt_linkFile.Text) == true)
                {
                    string[] s1 = txt_linkFile.Text.Split('+');
                    string s = "^XA\n^FO" + xMatrix + "," + yMatrix + "\n^BXN," + sizeMatrix + ",200\n^FD" + txt_linkFile.Text + "\n^FS" +
                               "\n^FO" + xRow + "," + yRow1 + "\n^A0," + heightChar + "," + widthChar + "\n^FD" + s1[0] + "\n^FS" +
                               "\n^FO" + xRow + "," + yRow2 + "\n^A0," + heightChar + "," + widthChar + "\n^FD+" + s1[1] + "\n^FS" +
                               "\n^FO" + xRow + "," + yRow3 + "\n^A0," + heightChar + "," + widthChar + "\n^FD+" + s1[2] + "\n^FS" +
                               "\n^FO" + xRow + "," + yRow4 + "\n^A0," + heightChar + "," + widthChar + "\n^FD+" + s1[3] + "\n^FS" +
