...
namespace ManageMaterialPBA
{
    public partial class NewCode : Form
    {
        ...
        public NewCode(DataGridView dgv, string user, string pass, string modell, string ngDung, string part, string dMon, string datTim, Form1 frm)
        {
            InitializeComponent();
            _dgv = dgv;
            _user = user;
        ...
        private void NewCode_Load(object sender, EventArgs e)
        {
            ...
            //Load ảnh minh họa
            pictureBox1.Image = new Bitmap(@Application.StartupPath + "\\Picture\\MasterTem.PNG");
            pictureBox1.SizeMode = PictureBoxSizeMode.StretchImage;

            //Load config
            StreamReader sr = new StreamReader(@Application.StartupPath + "\\PrinterConfig.ini");
        ...
        private void btn_Inthat_Click(object sender, EventArgs e)
        {
            ...
            {
                int err = 0;
                File.Delete(@Application.StartupPath + "\\Log\\Duplicate\\NewCode.log");
                File.Delete(@Application.StartupPath + "\\Log\\Duplicate\\FI-FO.log");
                //Luu New Code
                ...
                try
                {
                    ...
                    if (DialogResult.OK == pd.ShowDialog(this))
                    {
                        FileStream FS = new FileStream(@Application.StartupPath + "\\Log\\Duplicate\\NewCode.log", FileMode.Open);
                ...
                if (inSucess == true)
                {
                    #region
                    //Update history
                    FileStream fs1 = new FileStream(@Application.StartupPath + "\\History\\HistoryNVL.txt", FileMode.Append);
                    ...
                    try
                    {
                        ...
                        foreach (string fil in files)
                        {
                            ...
                            {
                                if (File.Exists(@Application.StartupPath + "\\Log\\Duplicate\\Old\\FI-FO.log"))
                                {
                                    File.Delete(@Application.StartupPath + "\\Log\\Duplicate\\Old\\FI-FO.log");
                                }
                                File.Move(@Application.StartupPath + "\\Log\\Duplicate\\FI-FO.log", @Application.StartupPath + "\\Log\\Duplicate\\Old\\FI-FO.log");
                            }

                            if (files[n].Contains("NewCode"))
                            {
                                StreamReader srLog = new StreamReader(@Application.StartupPath + "\\Log\\Duplicate\\NewCode.log");
                                FileStream fs = new FileStream(@Application.StartupPath + "\\Log\\Duplicate\\Old\\" + DateTime.Now.ToString("yyyyMMdd") + "_NewCode.log", FileMode.Append);
                                ...
                                File.Copy(@Application.StartupPath + "\\Log\\Duplicate\\NewCode.log", @"C:\Users\Administrator\Documents\" + DateTime.Now.ToString("yyyyMMdd") + "_NewCode.log");
                            }
                            File.Delete(fil);
                        jumpn:
                            n++;
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (cfrm == true && btnSave == true)
            {
                ...
                {
                    //Luu data
                    FileStream fs_p = new FileStream(@Application.StartupPath + "\\Print\\ChangePrinterConfig.ini", FileMode.Create);
                    ...
                    //So sanh
                    CompareAndHistory(@Application.StartupPath + "\\PrinterConfig.ini", @Application.StartupPath + "\\Print\\ChangePrinterConfig.ini"); 
                    
                    //Luu data
                    FileStream fs = new FileStream(@Application.StartupPath + "\\PrinterConfig.ini", FileMode.Create);                   
                    StreamWriter sw = new StreamWriter(fs);    
                    foreach (Control x in this.groupBox2.Controls)
                    {
                        if (x is TextBox)
            ...
            else
            {
                ...
                if (dtb1.up_FIFO() == true)
                {
                    ...
                    if (dtb1.upStokKtz(_dgv, _datTim) == true)
                    {
                        ...
                        bool chekExitFil = excel.checkExitLog(@Application.StartupPath + "\\History\\WH\\Order\\" + _dMon + "\\" + _datTim + "_" + _model + ".csv");
                        if (excel.exportCsvWHKtz(_dgv, @Application.StartupPath + "\\History\\WH\\Order\\" + _dMon + "\\" + _datTim + "_" + _model + ".csv", chekExitFil, _ngDung, "Nguoi_lay", 2, 1) == true)
                        {
                            MessageBox.Show("Tạo LogFile thành công!", "OrderWH", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            if (File.Exists(@Application.StartupPath + "\\tem\\" + _model + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadWH-KTZ.txt"))
                            {
                                File.Delete(@Application.StartupPath + "\\tem\\" + _model + "_" + DateTime.Now.ToString("yyMMdd") + "_" + "ReloadWH-KTZ.txt");
                            }
                            ...
                            try
                            {
                                ...
                                _dgv.Columns.Clear();
                                _frm.picBx1.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
                                _frm.picBx1.SizeMode = PictureBoxSizeMode.StretchImage;
                                _frm.picBx2.Image = new Bitmap(@Application.StartupPath + "\\Picture\\Default.PNG");
